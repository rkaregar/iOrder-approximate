...
// this class uses the initial biGraphs derived from each partition groups to generate pairwise clauses +
// the list of LHS and RHS chains (either initial ones or the pruned ones after mod2) to generate the other clauses
public class NewCPP2SATReducer {
    ...
    // return false if the reduction/solving has exceeded time limit
    ...
    public boolean newReduceAndSolve(int initialCardinalityCutoff, int connectedComponentMaxSizeLimit) {
        ...
        if (maxCardinality < initialCardinalityCutoff) {
            ...
            if ((Collections.max(simpleUndirGraphs.get(0).getConnectedComponents().stream().map(List::size).collect(Collectors.toList())) < connectedComponentMaxSizeLimit) &&
                ...
                MainClass.reductionRuntime += reductionTime;
                                    ...
                                    if (secondCC.get(l).side == 'l') {
                                        ...
                                        for (BipartiteNode secondEdgeRHS : bg.getAdj().get(secondEdgeLHS)) {
                                            ...
                                            if (shouldConsiderThisCCPair) {
                                                // ENFORCE an order between these values
                                                clauses.add(new int[]{ac, db});
                                                clauses.add(new int[]{ca, bd});
    ...
    // for each pair of tuples (i, u) and (j, v), these actually enforce an order between i, j and u, v, instead of
    // only checking for a swap not existing
    ...
    public boolean solveSAT() {
        ...
        try {
            ...
            MainClass.satRuntime += (System.nanoTime() - begin);
            MainClass.sats++;

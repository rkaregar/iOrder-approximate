...
public class ODAlgorithm {
    ...
    private int EUEIOD = 0, EUEIOC = 0, ECEIOD = 0, ECEIOC = 0;
    private int AUEIOD = 0, AUEIOC = 0, ACEIOD = 0, ACEIOC = 0;
    private int EUII = 0, ECII = 0, AUII = 0, ACII = 0;
    
    
    private boolean didFindUnconditionalEI;
    private boolean socHolds;  // help use the result of E/I aux to check I/I
    private int SOCctrAll, genSocCtr;
    
    public static ArrayList<HashMap<Long, String>> id2Val;  // Stores TupleID to actual value from file
    ...
    public void execute() throws AlgorithmExecutionException {
        ...
        while (!level1.isEmpty() && L <= numberAttributes) { //main loop of the algorithm
            ...
            MainClass.IOCPerLevelCount.add(SOCctrAll);
        }
        ...
        MainClass.writeMainStatToFile(EUEIOD + " = EUEIOD\n" + EUEIOC + " = EUEIOC\n" + ECEIOD +
                " = ECEIOD\n" + ECEIOC + " = ECEIOC\n" + AUEIOD + " = AUEIOD\n" + AUEIOC + " = AUEIOC\n" + ACEIOD +
                " = ACEIOD\n" + ACEIOC + " = ACEIOC\n" + EUII + " = EUII\n" + ECII + " = ECII\n" + AUII + " = AUII\n" +
                ACII + " = ACII", true);
        int odLevelCnt = 0, odTotalCnt = 0;
        for (int i = 0; i < MainClass.IOCPerLevelCount.size(); i++) {
            odLevelCnt += MainClass.IOCPerLevelCount.get(i) * (i + 2);
            odTotalCnt += MainClass.IOCPerLevelCount.get(i);
        }
        MainClass.writeMainStatToFile((double)odLevelCnt / odTotalCnt + " average OD level", false);
        MainClass.writeMainStatToFile(MainClass.IOCPerLevelCount + " ODs per level", false);
        
        MainClass.writeMainStatToFile("Reduction Runtime (ms) = " + (MainClass.reductionRuntime / 1_000_000), false);
        MainClass.writeMainStatToFile("# of SATs = " + MainClass.sats + ", SAT runtime (ms) = " + (MainClass.satRuntime / 1_000_000), false);
        MainClass.writeMainStatToFile("# of MAX-SATs = " + MainClass.maxSats + ", MAX-SAT runtime (ms) = " + (MainClass.maxSatRuntime / 1_000_000), false);
        
        MainClass.writeMainStatToFile("--------------------------------\n", false);
        ...
        MainClass.writeMainStatToFile(numberOfFD + " FD\n" + numberOfOD + " EEOC\n" +
                ...
                numberOfIIOC_cond + " IIOCcond\n" + numberOfIIOC_uncond + " IIOCuncond", false);
        ...
        // print special/general case orders:
        if (MainClass.FirstTimeRun) {
            ...
            for (SpecialScore specScore :
                ...
                if (MainClass.FullLogging)
                    MainClass.finalOCResults.add(messageToPrint + " " + MainClass.odList.get(MainClass.odList.size() - 1) + " " + specScore.order);
                else
                    MainClass.finalOCResults.add(messageToPrint + " " + MainClass.odList.get(MainClass.odList.size() - 1));
            }
            ...
            // These computations have been done above when storing the logs in the files
            System.out.println((double)odLevelCnt / odTotalCnt + " average OD level");
            System.out.println(MainClass.IOCPerLevelCount + " ODs per level");
        }
    }
            ...
            for (OpenBitSet oneAB : level1.get(X).getSwapCandidates()) {  //begin
                ...
                if (!(C_c_X_minus_B.equals(C_c_X_minus_B_Clone)) || !(C_c_X_minus_A.equals(C_c_X_minus_A_Clone))) {
                    ...
                    if (ODPass || BidODPass) {
                        ...
                        didFindUnconditionalEI = false;
                        socHolds = false;
                        
                        boolean gotEUEI = false, gotECEI = false, gotAUEI = false, gotACEI = false;
                        SOCOneSideSyntactic resEI1, resEI2;
                        // add the new trick for attribute annotation
                        if (MainClass.isColumnActive.get(A_index - 1)) {
                            resEI1 = EIExecAux(X_minus_AB, oneAB, A_index, B_index, PI_X_TAU_A_1, PI_X_TAU_B_1, xDeterminesA,
                                    A, B, datasetName, strippedPartition_X_minus_AB, L, "(R->L)", true, false);
                            gotEUEI = resEI1.holds && !resEI1.isOrderConditional;
                            gotECEI = resEI1.holds && resEI1.isOrderConditional;
                            if (!gotEUEI && !MainClass.OnlyExactOCs) {
                                resEI1 = EIExecAux(X_minus_AB, oneAB, A_index, B_index, PI_X_TAU_A_1, PI_X_TAU_B_1, xDeterminesA,
                                        A, B, datasetName, strippedPartition_X_minus_AB, L, "(R->L)", false, resEI1.holds);
                                gotAUEI = resEI1.holds && !resEI1.isOrderConditional;
                                gotACEI = resEI1.holds && resEI1.isOrderConditional;
                            }
                        }
                        if (MainClass.isColumnActive.get(B_index - 1)) {
                            resEI2 = EIExecAux(X_minus_AB, oneAB, B_index, A_index, PI_X_TAU_B_1, PI_X_TAU_A_1, xDeterminesB,
                                    B, A, datasetName, strippedPartition_X_minus_AB, L, "(L->R)", true, false);
                            gotEUEI = gotEUEI || (resEI2.holds && !resEI2.isOrderConditional);
                            gotECEI = gotECEI || (resEI2.holds && resEI2.isOrderConditional);
                            if (!(resEI2.holds && !resEI2.isOrderConditional) && !MainClass.OnlyExactOCs) {
                                resEI2 = EIExecAux(X_minus_AB, oneAB, B_index, A_index, PI_X_TAU_B_1, PI_X_TAU_A_1, xDeterminesB,
                                        B, A, datasetName, strippedPartition_X_minus_AB, L, "(L->R)", false, resEI2.holds);
                                gotAUEI = gotAUEI || (resEI2.holds && !resEI2.isOrderConditional);
                                gotACEI = gotACEI || (resEI2.holds && resEI2.isOrderConditional);
                            }
                        }
                        ...
                        if (MainClass.isColumnActive.get(A_index - 1) && MainClass.isColumnActive.get(B_index - 1) &&
                            ...
                            SOCGeneralCase resII;
                            resII = IIExecAux(X_minus_AB, oneAB, A_index, B_index, PI_X_TAU_A_1, PI_X_TAU_B_1, xDeterminesA, xDeterminesB,
                                    A, B, datasetName, strippedPartition_X_minus_AB, flakyMap, L, true, false, gotECEI, gotACEI);
                            if ((!(resII.holds && !resII.isTheOrderConditional)) && (!gotAUEI) && !MainClass.OnlyExactOCs) {  // otherwise skip approximate
                                resII = IIExecAux(X_minus_AB, oneAB, A_index, B_index, PI_X_TAU_A_1, PI_X_TAU_B_1, xDeterminesA, xDeterminesB,
                                        A, B, datasetName, strippedPartition_X_minus_AB, flakyMap, L, false, resII.holds, gotECEI, gotACEI);
                            }
                        }
                        // prune
                        if (socHolds) {
    ...
    // IMPORTANT: This function always assumes we want to find the order from R -> L, i.e., B to A
    // to work in the other direction, outputs should be flipped when calling the function
    private SOCOneSideSyntactic EIExecAux(OpenBitSet X_minus_AB, OpenBitSet oneAB, int A_index, int B_index,
                           ObjectBigArrayBigList<ObjectBigArrayBigList<LongBigArrayBigList>> PI_X_TAU_A_1,
                           ObjectBigArrayBigList<ObjectBigArrayBigList<LongBigArrayBigList>> PI_X_TAU_B_1,
                           boolean xDeterminesTarget, OpenBitSet A, OpenBitSet B, String datasetName,
                           ObjectBigArrayBigList<LongBigArrayBigList> strippedPartition_X_minus_AB,
                           int L, String direction, boolean checkExact, boolean onlyUnconditional) {
        boolean isOD;
        String ocOrOd;
        SOCOneSideSyntactic specSocMiner;
        List<Map<Long, Set<Long>>> specOrders;
        long duration;
        
        if (xDeterminesTarget) {
            isOD = true;
            ocOrOd = "OD";
        } else {
            isOD = false;
        ...
        printOpenBitSetNames("Inspecting E/I " + ocOrOd + " " + direction + ": ", X_minus_AB, oneAB);
    
        long startTime = System.nanoTime();
        specSocMiner = new SOCOneSideSyntactic(TAU_reverseMap, A_index, B_index, isOD, PI_X_TAU_B_1, PI_X_TAU_A_1);
        
        if (checkExact)
            specOrders = specSocMiner.executeExact();
        else
            specOrders = specSocMiner.executeApprox(onlyUnconditional);
        ...
        if (specOrders != null) {
            printOpenBitSetNames(SOCctrAll + " - E/I " + ocOrOd + " " + direction, X_minus_AB, oneAB);
            socHolds = true;
            SOCctrAll++;
            
            if (specSocMiner.isOrderConditional) {
                if (isOD) {
                    if (specSocMiner.isOrderApproximate)
                        ACEIOD++;
                    else
                        ECEIOD++;
                    numberOfEIOD_cond++;
            ...
            SpecialScoreList.add(new SpecialScore(specialScores[0], specialScores[1], direction, isOD, specSocMiner.isOrderConditional, X_minus_AB, oneAB, orderStr));
        }
        return specSocMiner;
    }
    
    private SOCGeneralCase IIExecAux(OpenBitSet X_minus_AB, OpenBitSet oneAB, int A_index, int B_index,
                          ObjectBigArrayBigList<ObjectBigArrayBigList<LongBigArrayBigList>> PI_X_TAU_A_1,
                          ObjectBigArrayBigList<ObjectBigArrayBigList<LongBigArrayBigList>> PI_X_TAU_B_1,
                          boolean xDeterminesA, boolean xDeterminesB, OpenBitSet A, OpenBitSet B, String datasetName,
                          ObjectBigArrayBigList<LongBigArrayBigList> strippedPartition_X_minus_AB,
                          Map<Long, Set<WeightedEdge>> flakyMap, int L, boolean checkExact, boolean onlyUnconditional,
                          boolean gotECEI, boolean gotACEI) {
        long duration;
        ...
        // LHS/RHS seem to be the `second` dimension in genOrder
        ArrayList<List<Map<Long, Set<Long>>>> genOrder;
        if (checkExact)
            genOrder = genSocMiner.executeExact();
        else
            genOrder = genSocMiner.executeApprox(onlyUnconditional);
        ...
        if (genOrder != null) {
            ...
            if ((!genSocMiner.isTheOrderApproximate && (!genSocMiner.isTheOrderConditional || !gotECEI)) ||
                    (genSocMiner.isTheOrderApproximate && (!genSocMiner.isTheOrderConditional || !gotACEI))) {
                ...
                if (scores[0][0] > 0d || scores[0][1] > 0d) {
                    ...
                    if (genSocMiner.isTheOrderConditional) {
                        if (genSocMiner.isTheOrderApproximate)
                            ACII++;
                        else
                            ECII++;
                        numberOfIIOC_cond++;
    ...
    private ObjectArrayList<Object2ObjectOpenHashMap<Object, LongBigArrayBigList>> loadData() {
        ...
        this.numberAttributes = columnNamesList.size();
        this.tableName = "";
    ...
    public static String orderToString(List<Map<Long, Set<Long>>> order, int colIndex) {
    ...
    public static List<Long> totalOrder2List(Map<Long, Set<Long>> totalOrder) {
        int n = totalOrder.keySet().size();
        List<Long> res = new ArrayList<>(Collections.nCopies(n, -1L));
        for (Long it : totalOrder.keySet()) {
            int index = n - 1 - totalOrder.get(it).size();
            if (res.get(index) >= 0) {
                System.out.println("ERROR!!!!! THIS IS NOT A TOTAL ORDER!!!!");
            } else {
                res.set(index, it);
            }
        }
    ...
    public static String totalOrder2String(List<Long> totalOrder, int colIndex) {
        StringBuilder res = new StringBuilder();
        Map<Long, String> curId2Val = id2Val.get(colIndex - 1);
    
        for (Long it : totalOrder) {
            res.append(curId2Val.get(it) + " < ");
        }
        return res.toString();
    }
}

...
public class SOCGeneralCase {
    ...
    public List<List<Map<Long, Set<Long>>>> eachPGChains;  // each PG has one chain or 'null'. second dimension is LHS/RHS
    public boolean holds;
    public boolean isTheOrderConditional; // depending on the config file and the found order, this will be set to sth
    public boolean isTheOrderApproximate = false;
    ...
    public ArrayList<List<Map<Long, Set<Long>>>> executeApprox(boolean onlyCheckUnconditional) {
        isTheOrderConditional = true;
        long startTime = System.nanoTime();
        deriveChains();
    
        System.out.println("ii let's go");
        ApproxIISATReducer approxIISATReducer = new ApproxIISATReducer(allBipGraphs, eachPGChains, A_index, B_index, MainClass.ApproxThreshold, onlyCheckUnconditional);
        approxIISATReducer.reduceAndSolve(1000, 1000);
    
        isTheOrderApproximate = true;
        isTheOrderConditional = approxIISATReducer.isTheOrderConditional;
        approxScores = approxIISATReducer.approxScores;
        return approxIISATReducer.finalOrder;
        // TODO should update logging for this and the two other things (and also the E/I case most likely)
        //  also may need to add the hopeless MainClass.OnlyCheckConditionalIIOC thing
    
    }
    
    // What I have here for now just derives the chains (as long as each individual chain is valid)
    public ArrayList<List<Map<Long, Set<Long>>>> executeExact() {
        isTheOrderConditional = true;
        ...
        if (allChains != null) {
            ...
            if (!MainClass.OnlyCheckConditionalIIOC) {
                // try including mod1 again
                Map<Long, Long> mergeTrail = new HashMap<>();
                flakyMap = module1(allChains, mergeTrail); // you can count the number of iterations it took
                // for module1 to converge to settle on a set of polarity groups
                
                if (flakyMap != null) {  // we successfully passed module 1 [Reza]
                    flakyMap = mapToUltimateDest(mergeTrail, flakyMap);
                    ODAlgorithm.flakeCount = flakyMap.size();
                    if (flakyMap.size() == 0) {
                        // valid after module1, don't need to continue to module2
                        // the order you want is in allChains. you can just print it or something for now
    ...
    private boolean deriveChains() { //derives chains and returns true if all individual chains were acyclic, and false otherwise
        ...
        eachPGChains = new ArrayList<>();
        
        boolean shouldFalse = false;
        ...
        for (int i = 0; i < TAU_XA.size64(); i++) { // or TAU_XB -- iterating over eq. classes on X
            ...
            bipGraph.initialize();
            // todo this changed as well (swapped their order)
            allBipGraphs.add(bipGraph);
            
            if (bipGraph.isCyclic()) {
                shouldFalse = true;
                // add two 'null' chains corresponding to this PG
                eachPGChains.add(new ArrayList<>(Arrays.asList(null, null)));
            } else {
                // in chains, second dimension is for LHS/RHS (chains.get(i).get(0/1)) [Reza]
                // so apparently, the same thing also holds for allChains
                ArrayList<ArrayList<Map<Long, Set<Long>>>> chains = bipGraph.getChainAdjs();
                allChains.addAll(chains);
                eachPGChains.add(mergeChains(chains));
            }
        }
        return !shouldFalse;
    }
    
    // takes multiple (probably) disjoint chains corresponding to a single PG and merges them together
    private List<Map<Long, Set<Long>>> mergeChains (ArrayList<ArrayList<Map<Long, Set<Long>>>> chains) {
        List<Map<Long, Set<Long>>> res = new ArrayList<>(Arrays.asList(new HashMap<>(), new HashMap<>()));
        for (ArrayList<Map<Long, Set<Long>>> doubleChain : chains) {
            for (int side = 0; side < doubleChain.size(); side++) {
                Map<Long, Set<Long>> ch = doubleChain.get(side);
                for (Long k : ch.keySet()) {
                    if (!res.get(side).containsKey(k))
                        res.get(side).put(k, new HashSet<>());
                    for (Long v : ch.get(k))
                        res.get(side).get(k).add(v);
    ...
    private Map<Long, Set<WeightedEdge>> module1(ArrayList<List<Map<Long, Set<Long>>>> allChains, Map<Long, Long> mergeTrail) {
        ...
        //for now you can assume that everything will be accessible from this function and move forward:
        // when you empty a component you add its index (in allChains) to this list
                    ...
                    while (j < allChains.size()) {
                        if (!emptyComps.contains(j)) {
                            ...
                            if (commonKeysLHS.size() >= 2) {
                                ...
                                // if cycle free only in one direction, ...
                                if (!isCycleFreeReverseLHS && !isCycleFreeStraightLHS) { // none of them are cycle free
                                    ...
                                    if (!(new Graph2(mergeStraightRHS)).isCyclic()) {
                                        // you're just supposed to merge them. not return anything yet!
    ...
    private Map<Long, Set<Long>> mergeAdjs(Map<Long, Set<Long>> adj1, Map<Long, Set<Long>> adj2, boolean reverse) {
        ...
        if (reverse) {
            adj2 = reverseAdj(adj2); // adj2 will be emptied after this function is done anyways
        }
